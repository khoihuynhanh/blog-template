...
import { addPost, cancelEditingPost, updatePost } from "../feature/BlogSlice"
...
const CreatePost = () => {
    const editingPost = useSelector(state => state.blogSlice.editingPost)
    ...
    const handleSubmit = async (e) => {
        ...
        if (editingPost) {
            ...
            try {
                ...
                unwrapResult(res) // lay loi hoac data tu dispatch tra ve thi unwrapResult
                setFormData(initialState)
                if (errorForm) {             // xoa loi
                    setErrorForm(null)
                }
    ...
    return (
        <form onSubmit={handleSubmit} onReset={handleCancel}>
            ...
            <div className="mb-6">
                <label htmlFor="publishDate" className={`mb-2 block text-sm font-medium dark:text-gray-300 ${errorForm?.publishDate ? 'text-red-700' : 'text-gray-900'}`}>Publish Date</label>
                <input
                    type="datetime-local"
                    id="publishDate"
                    className={`block w-60 rounded-lg border focus:outline-none ${errorForm?.publishDate ? 'border-red-500 bg-red-50 text-red-900 placeholder-red-700 focus:border-red-500 focus:ring-red-500 p-2.5' : ' border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-blue-500'}`}
                    placeholder="Title"
                    required
                ...
                {errorForm?.publishDate && (
                    <p className='mt-2 text-sm text-red-6000'>
            ...
            <div className="mb-6 flex items-center gap-10">
                <div className='flex items-center'>
                    ...
                    />
                    <label htmlFor="publish" className="ml-3 text-sm font-medium text-gray-900">Publish</label>
                </div>
                <div>
                    {editingPost ? (
                        <>
